...
/// An auxiliary function for the Weak-Zero SIV test. Return true if \p AR
/// overlaps with \p Const iff at the last iteration of the loop, where \p AR is
/// an affine AddRec expression, \p Const is a loop invariant constant, and \p
/// UpperBound is the exact backedge-taken count of the loop.
static bool isOverlapLastWeakZeroSIV(const SCEVAddRecExpr *AR,
                                     const SCEV *Const, ScalarEvolution &SE,
                                     const SCEV *UpperBound) {
  return AR->hasNoSignedWrap() &&
         SE.isKnownNonZero(AR->getStepRecurrence(SE)) &&
         SE.isKnownPredicate(CmpInst::ICMP_EQ,
                             AR->evaluateAtIteration(UpperBound, SE), Const);
};
...
bool DependenceInfo::weakZeroSrcSIVtest(const SCEV *SrcConst,
  ...
  if (const SCEV *UpperBound =
          collectUpperBound(Dst->getLoop(), Dst->getType())) {
    bool OverlapAtLast =
        isOverlapLastWeakZeroSIV(Dst, SrcConst, *SE, UpperBound);
    if (OverlapAtLast) {
...
bool DependenceInfo::weakZeroDstSIVtest(const SCEVAddRecExpr *Src,
  ...
  if (const SCEV *UpperBound =
          collectUpperBound(Src->getLoop(), Src->getType())) {
    bool OverlapAtLast =
        isOverlapLastWeakZeroSIV(Src, DstConst, *SE, UpperBound);
    if (OverlapAtLast) {
      if (Level < CommonLevels) {
        Result.DV[Level].Direction &= Dependence::DVEntry::GE;
        ++WeakZeroSIVsuccesses;
